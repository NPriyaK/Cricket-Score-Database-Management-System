...
ALTER TABLE PointsTable ADD CONSTRAINT RejectWrongEdition CHECK (CheckEdition(TeamID, Edition)>=0); 
...
ALTER TABLE Bowler ADD CONSTRAINT RejectWicketKeeper CHECK (CheckWicketKeeper(PlayerID)=0); 
...
ALTER TABLE WicketKeeper ADD CONSTRAINT RejectBowler CHECK (CheckBowler(PlayerID)=0);
...
ALTER TABLE TournamentAwards ADD CONSTRAINT RejectPlayerAward CHECK (CheckPlayerTypeAwards(PlayerID, CategoryID)>0);